...
import java.util.TimeZone;
 ...
 public class MyReservations extends Fragment {
	...
	TextView errorText,dayView,listView,monthView,selectedPeriod;
	View dayViewseperator,listViewseperator,monthViewseperator;
	...
	GridLayout monthGridView,dayGridView,hoursInday,days;
	LinearLayout listGridView;
	...
	public MyReservations(Date selectedDate) {
	...
	public MyReservations(Context ctx){
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,Bundle savedInstanceState) {
		...
		listGridView = (LinearLayout)rootView.findViewById(R.id.listGridView);
		...
		/*listGridView.setOnTouchListener(new View.OnTouchListener() {
		    @Override
		    public boolean onTouch(View v, MotionEvent event) {
		    	v.performClick();
		        return gestureDetector.onTouchEvent(event);
		...
		listView.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				//setSelectedDay(listViewseperator,listView, selectedDayType);
				Calendar cal = Calendar.getInstance();
				cal.setTime(new Date());
				//cal.set(Calendar.DATE, 1);
				schedMonthStartDate =cal.getTime();
				cal.add(Calendar.MONTH, 3);
				schedMonthEndDate = cal.getTime();
				new RunInBackground(LOAD_RESERVATIONS,null,null,null,-1,0,false).execute();
				//selectedDayType = DATE_TYPE_WEEKLY;
				//setSelectedDay(listViewseperator,listView, selectedDayType);
			}
	...
	private void loadNextData(int multiplier){
		if(selectedDayType == DATE_TYPE_DAILY){
			...
			Calendar cal = (Calendar) currCalendar.clone();
	...
	private void setSelectedDay(View selSepView,TextView selTxtView,int type){
		...
		listViewseperator.setBackgroundResource(R.color.white);
		monthViewseperator.setBackgroundResource(R.color.white);

		
		dayView.setTextColor(appresources.getColor(R.color.black));
		listView.setTextColor(appresources.getColor(R.color.black));
	...
	private String getMeetingDetalilsListString(ReservationModel model){
		String detl ="";
		if(model != null){
			detl = model.getReservationName()+"\n";
			detl +=model.getResourceName()+"\n";
			detl +=mdyHMFormat.format(model.getStartDate())+" - "+mdyHMFormat.format(model.getEndTime());
		}
		return detl;
	...
	private void loadWeeklyData() {
		...
		selectedPeriod.setText("All reservations");
		listGridView.removeAllViews();
		float scale = appresources.getDisplayMetrics().density;
		List<ReservationModel> reservatonList1 = new ArrayList<ReservationModel>();
		if(reservationMapForDays != null){
			for(String day : reservationMapForDays.keySet()){
				Map<Integer, List<ReservationModel>> map = reservationMapForDays.get(day);
				if (map != null) {
					
					for (int key : map.keySet()) {
						reservatonList1.addAll(map.get(key));
					}
				}
			}
		}
		...
		if(reservatonList1 != null && reservatonList1.size()>0){
			for(ReservationModel resModel : reservatonList1){
				if(resModel.getReservedByUser() != Utilities.loggedInUser.getUserId()){
					continue;
				}
				int startMin = 0;
				...
				TextView textView = new TextView(mContext);
				LayoutParams parm = new LayoutParams(LayoutParams.MATCH_PARENT,(int)(70*scale));
				parm.setMargins((int)(10*scale), (int)(5*scale), (int)(10*scale), 0);
				textView.setLayoutParams(parm);
				textView.setBackgroundResource(R.drawable.curved_border_filled);
				textView.setText(getMeetingDetalilsListString(resModel));
				...
				textView.bringToFront();
				listGridView.addView(textView);
				
			}
		}else{
			TextView textView = new TextView(mContext);
			LayoutParams parm = new LayoutParams(LayoutParams.MATCH_PARENT,(int)(30*scale));
			parm.setMargins((int)(10*scale), (int)(5*scale), (int)(10*scale), 0);
			textView.setLayoutParams(parm);
...
*/					//}
	...
	public void onSwipeRight() {
		if(selectedDayType != DATE_TYPE_WEEKLY){
			loadNextData(-1);
		}
	}
	...
	private void setMonth(Date dat){
		currCalendar.setTime(dat);
		Calendar startDate = (Calendar) currCalendar.clone();
		int currMonth = startDate.get(Calendar.MONTH);
		int noOFWeeks = 4;
		startDate.set(Calendar.DAY_OF_MONTH, 1);
